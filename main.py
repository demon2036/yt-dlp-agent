#!/usr/bin/env python3
"""简洁的 yt-dlp 下载工具"""

import os
import argparse


def build_command(urls, audio_only=True, use_proxy=True):
    """
    ╔═══════════════════════════════════════════════════════════╗
    ║ 构建 yt-dlp 命令                                           ║
    ║ • 支持单个或多个 URL 批量下载                               ║
    ║ • 默认启用代理和音频提取                                   ║
    ╚═══════════════════════════════════════════════════════════╝
    """
    # ─────────────────────────────────────────────────────────
    # 基础命令：并发下载 + 路径模板 + Cookie 认证
    # ─────────────────────────────────────────────────────────
    cmd = [
        "yt-dlp",
        "-N 16",
        "-o 'yt_dlp/%(uploader)s/%(playlist|NA).20s/%(playlist_index)02d %(title).60s.%(ext)s'",
        "--cookies-from-browser firefox",
        "--hls-prefer-native",
    ]

    # ─────────────────────────────────────────────────────────
    # 音频/视频模式
    # ─────────────────────────────────────────────────────────
    if audio_only:
        cmd.extend(["-x", "--audio-format mp3"])
    else:
        cmd.extend(["--write-sub", "--write-auto-sub", "--embed-sub"])

    # ─────────────────────────────────────────────────────────
    # 代理配置
    # ─────────────────────────────────────────────────────────
    if use_proxy:
        cmd.append("--proxy socks5://127.0.0.1:1089/")

    # ─────────────────────────────────────────────────────────
    # URL 列表（支持单个或多个，统一处理）
    # ─────────────────────────────────────────────────────────
    if isinstance(urls, str):
        cmd.append(urls)
    else:
        cmd.extend(urls)

    return " ".join(cmd)


def main():
    """
    ╔═══════════════════════════════════════════════════════════╗
    ║ 主函数：解析参数并执行下载                                 ║
    ╚═══════════════════════════════════════════════════════════╝
    """
    parser = argparse.ArgumentParser(
        description='yt-dlp 批量下载工具',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
示例:
  %(prog)s URL1 URL2 URL3                    # 下载多个视频的音频
  %(prog)s -v URL1 URL2                      # 下载视频（含字幕）
  %(prog)s --no-proxy URL                    # 不使用代理
        '''
    )

    # ─────────────────────────────────────────────────────────
    # 参数定义
    # ─────────────────────────────────────────────────────────
    parser.add_argument('urls', nargs='+',
                        help='一个或多个视频 URL')
    parser.add_argument('-v', '--video', action='store_true',
                        help='下载视频（默认只下载音频）')
    parser.add_argument('--no-proxy', action='store_true',
                        help='不使用代理（默认使用代理）')

    args = parser.parse_args()

    # ─────────────────────────────────────────────────────────
    # 构建并执行命令
    # ─────────────────────────────────────────────────────────
    cmd = build_command(
        args.urls,
        audio_only=not args.video,
        use_proxy=not args.no_proxy
    )

    print(f"📥 准备下载 {len(args.urls)} 个视频")
    print(f"🔧 执行命令: {cmd}\n")
    print("─" * 60)

    os.system(cmd)


if __name__ == '__main__':
    main()